...
function rankedQueueKey(playerCount, deckCount = deckCountFor(playerCount)){ return `${currentSeasonId()}:${playerCount}:${deckCount}`; }
function checkRankedQueue(playerCount, deckCount = deckCountFor(playerCount)){ const key=rankedQueueKey(playerCount, deckCount), q=rankedQueues.get(key)||[]; const live=q.filter(x=>io.sockets.sockets.has(x.socketId)); rankedQueues.set(key,live); if(live.length<playerCount)return;
...
function restorePersistentState() {
  ...
  try {
    ...
    for (const saved of raw.rooms || []) {
      ...
      saved.bidHistory ||= []; saved.trickHistory ||= []; saved.integrity ||= {deckVerified:false,scoreVerified:true};
...
function ruleSummaryFor(room) {
  ...
  return `${room.playerCount}P · ${d} deck${d===2?"s":""} · ${cards} cards each · ${totalPointsFor(room)} pts · Bid ${min}–${max}`;
}
...
function finishRound(room) {
  ...
  room.roundSummary={made,bid:contract,bidderPoints,defensePoints,mvpIndex,mvpName:room.players[mvpIndex]?.name||"Player",mvpPoints:room.players[mvpIndex]?.roundPoints||0,totalPoints:totalPointsFor(room),auditId:room.auditId,integrity:{...(room.integrity||{})}};
  room.phase = "roundEnd";
  addLog(room, made
...
io.on("connection", socket => {
  ...
  socket.on("rankedJoin",(payload={},ack)=>{const me=userForSocket(socket);if(!me)return ack?.({ok:false,error:"Sign in to play ranked."});if(findRoomBySocket(socket.id))return ack?.({ok:false,error:"Leave your room first."});const pc=[3,4,5,6,7,8].includes(Number(payload.playerCount))?Number(payload.playerCount):4;const dc=pc===4&&Number(payload.deckCount)===2?2:deckCountFor(pc);removeFromRankedQueue(socket.id);const key=rankedQueueKey(pc,dc),q=rankedQueues.get(key)||[];q.push({socketId:socket.id,userId:me.id,joinedAt:Date.now(),deckCount:dc});rankedQueues.set(key,q);ack?.({ok:true,position:q.length,needed:pc,season:currentSeasonId(),rating:ensureSeason(me).rating,deckCount:dc});io.to(socket.id).emit("rankedQueueStatus",{position:q.length,needed:pc,deckCount:dc});checkRankedQueue(pc,dc);});
