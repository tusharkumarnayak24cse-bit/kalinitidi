...
function showGame(){
  $("homeScreen").classList.add("hidden"); $("profilePanel").classList.add("hidden"); $("discoveryPanel")?.classList.add("hidden"); $("gameScreen").classList.remove("hidden");
...
function renderLobby(){
  ...
  if(readyBtn){readyBtn.classList.toggle("hidden",state.spectator);const me=state.players[state.viewerIndex];readyBtn.textContent=me?.ready?"✓ Ready":"○ Mark Ready";readyBtn.classList.toggle("is-ready",!!me?.ready);}
...
function renderHand(){
  ...
  state.hand.forEach((card,cardIndex)=>{
    ...
    if(can){
      ...
      const play=()=>{haptic(20);animateCardPlay(el,()=>socket.emit("playCard",{cardId:card.id},res=>{if(res&&!res.ok){el.classList.remove("card-launching");toast(res.error);}}));};
...
$("createForm").addEventListener("submit",e=>{
  ...
  socket.emit("createRoom",{name:p.name,avatar:p.avatar,playerCount:Number($("playerCount").value),isPublic:Boolean($("publicRoomToggle")?.checked),botDifficulty:$("botDifficulty")?.value||"normal"},res=>{if(!res?.ok)return toast(res?.error||"Could not create room.");saveSession(res.code,res.reconnectToken);});
});
...
socket.on("state",next=>{
  ...
  if(prev&&prev.phase==="playing"&&next.phase==="roundEnd"){beep("win");haptic([40,35,80]);recordRoundStats(next);}
...
socket.on("connect",()=>{setNetStatus("online","● Online");tryResumeSession(false);socket.emit("rtcConfig",{},res=>{if(res?.ok&&Array.isArray(res.iceServers))rtcConfig={iceServers:res.iceServers};});refreshPublicRooms();});
...
function refreshPublicRooms(){
  if(!socket.connected)return;socket.emit("listPublicRooms",{},res=>{
    ...
    const cards=[];
    open.forEach(r=>cards.push(`<div class="public-room-card"><div><strong>${escapeHtml(r.hostName)}'s table</strong><span>${r.joined}/${r.playerCount} players · ${r.botDifficulty} bots · ${escapeHtml(r.code)}</span></div><button type="button" class="join-public-btn primary" data-code="${r.code}">Join</button></div>`));
    ...
    root.querySelectorAll(".join-public-btn").forEach(b=>b.addEventListener("click",()=>joinPublicRoom(b.dataset.code)));
    root.querySelectorAll(".spectate-public-btn").forEach(b=>b.addEventListener("click",()=>spectateRoom(b.dataset.code)));
  });
}
function joinPublicRoom(code){const p=currentProfile();saveProfile();socket.emit("joinRoom",{name:p.name,avatar:p.avatar,code},res=>{if(!res?.ok)return toast(res?.error||"Could not join room.");saveSession(res.code,res.reconnectToken);});}
...
function startQuickMatch(){const p=currentProfile();saveProfile();socket.emit("quickMatch",{name:p.name,avatar:p.avatar,playerCount:Number($("playerCount")?.value||8),botDifficulty:$("botDifficulty")?.value||"normal"},res=>{if(!res?.ok)return toast(res?.error||"Could not find a match.");saveSession(res.code,res.reconnectToken);toast("Quick Match table found.");});}
