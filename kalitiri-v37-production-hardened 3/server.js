...
const SESSION_TTL_DAYS = Math.max(1, Math.min(90, Number(process.env.SESSION_TTL_DAYS || 30)));
const SESSION_TTL_MS = SESSION_TTL_DAYS * 24 * 60 * 60 * 1000;
const MAX_SESSIONS_PER_USER = Math.max(1, Math.min(10, Number(process.env.MAX_SESSIONS_PER_USER || 5)));
...
app.use((req, res, next) => {
  ...
  res.setHeader("Content-Security-Policy", "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self' https: wss:; media-src 'self' blob:; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'");
  next();
});
app.get("/healthz", (_req, res) => res.status(200).json({ ok: true, region:REGION_ID, version:GAME_VERSION, storage:accountStore?.status?.().mode || "initializing" }));
...
const sessionStore = createSessionStore({ ttlMs:SESSION_TTL_MS, maxSessionsPerUser:MAX_SESSIONS_PER_USER });
const accountStore = createAccountStore({ databaseUrl:DATABASE_URL, filePath:ACCOUNTS_FILE, isProd:IS_PROD, logger:console });
if (IS_PROD && REQUIRE_DATABASE && !DATABASE_URL) throw new Error("DATABASE_URL is required in production. Set it before deploying this build.");
if (IS_PROD && !DATABASE_URL) console.warn("WARNING: DATABASE_URL is not set. Account data is not safe on an ephemeral Render filesystem.");
if (IS_PROD && ADMIN_KEY && ADMIN_KEY.length < 24) console.warn("WARNING: ADMIN_KEY should be at least 24 random characters.");
...
function scryptPassword(password, salt) {
  return new Promise((resolve, reject) => crypto.scrypt(String(password), salt, 64, (error, derivedKey) => error ? reject(error) : resolve(derivedKey)));
}
...
async function verifyPassword(password, u) {
  try {
    const a = await scryptPassword(password, u.salt);
    const b = Buffer.from(String(u.passwordHash || ""), "hex");
    return a.length === b.length && crypto.timingSafeEqual(a, b);
  } catch { return false; }
}
function accountSnapshot() {
  return { version:2, savedAt:Date.now(), users:[...users.values()], replays:replays.slice(-500), tournaments:[...tournaments.values()], sessions:sessionStore.serialize() };
}
function persistAccountsSoon() { accountStore.schedule(accountSnapshot, 250); }
async function restoreAccounts() {
  try {
    const raw = await accountStore.restore();
    if (!raw) { console.log(`Account storage ready (${accountStore.mode()}); no snapshot found.`); return; }
    ...
    const restoredSessions = sessionStore.restore(raw.sessions||[]);
    console.log(`Restored ${users.size} account(s), ${restoredSessions} active session(s) from ${accountStore.mode()}.`);
  } catch(e) {
    console.error("Account restore failed:", e.message);
    if (IS_PROD && DATABASE_URL) throw e;
  }
}
function findUserByUsername(username) { const n=normalizeUsername(username); return [...users.values()].find(u=>u.username===n) || null; }
function issueSession(userId) { const issued=sessionStore.issue(userId); persistAccountsSoon(); return issued; }
...
function secureSecretEqual(a,b){
  const aa=Buffer.from(String(a||""),"utf8"), bb=Buffer.from(String(b||""),"utf8");
  return aa.length===bb.length && aa.length>0 && crypto.timingSafeEqual(aa,bb);
}
function adminAllowed(req){ return Boolean(ADMIN_KEY) && secureSecretEqual(req.headers["x-admin-key"], ADMIN_KEY); }
...
io.on("connection", socket => {
  ...
  socket.on("roomQr",async(payload={},ack)=>{try{const code=String(payload.code||findRoomBySocket(socket.id)?.room?.code||"").toUpperCase();if(!code)return ack?.({ok:false,error:"Room unavailable."});const base=String(process.env.PUBLIC_GAME_URL||process.env.RENDER_EXTERNAL_URL||"https://tusharevent-2.onrender.com").replace(/\/$/,"");const url=`${base}/?room=${encodeURIComponent(code)}`;const dataUrl=await QRCode.toDataURL(url,{margin:1,width:320,errorCorrectionLevel:"M"});ack?.({ok:true,code,url,dataUrl});}catch(e){ack?.({ok:false,error:"QR unavailable."});}});
  ...
  socket.on("accountRegister", async (payload={},ack)=>{
    if(!takeRate(socket,"accountRegister",5,60_000,"ip"))return rejectRate(ack);
    const username=normalizeUsername(payload.username), password=String(payload.password||""); if(username.length<3)return ack?.({ok:false,error:"Username needs at least 3 characters."}); if(password.length<8)return ack?.({ok:false,error:"Password needs at least 8 characters."}); if(password.length>128)return ack?.({ok:false,error:"Password is too long."}); if(findUserByUsername(username))return ack?.({ok:false,error:"Username already exists."});
    const {salt,hash}=await hashPassword(password), id=crypto.randomUUID(); const u={id,username,displayName:cleanName(payload.displayName||username),avatar:cleanAvatar(payload.avatar),salt,passwordHash:hash,createdAt:Date.now(),friends:[],friendRequests:[],achievements:[],stats:{rounds:0,wins:0,losses:0,points:0,tricksWon:0,highestBid:0},seasons:{},settings:{theme:"classic",cardBack:"emerald",largeText:false,highContrast:false},daily:{}}; users.set(id,u);ensureSeason(u);const issued=issueSession(id);socket.data.userId=id;socket.data.accountSessionHash=issued.sessionHash;persistAccountsSoon();ack?.({ok:true,token:issued.token,expiresAt:issued.expiresAt,user:publicUser(u)});
  });
  socket.on("accountLogin",async(payload={},ack)=>{ if(!takeRate(socket,"accountLogin",10,60_000,"ip"))return rejectRate(ack); const password=String(payload.password||"");if(password.length>128)return ack?.({ok:false,error:"Incorrect username or password."});const u=findUserByUsername(payload.username);if(u?.bannedUntil>Date.now())return ack?.({ok:false,error:`Account suspended: ${u.banReason||"moderator action"}`});if(!u||!(await verifyPassword(password,u)))return ack?.({ok:false,error:"Incorrect username or password."});const issued=issueSession(u.id);socket.data.userId=u.id;socket.data.accountSessionHash=issued.sessionHash;ack?.({ok:true,token:issued.token,expiresAt:issued.expiresAt,user:publicUser(u)}); });
  socket.on("accountResume",(payload={},ack)=>{const sess=sessionStore.validate(String(payload.token||""));const u=sess&&users.get(sess.userId);if(!u||u.bannedUntil>Date.now()){if(sess?.sessionHash)sessionStore.revokeHash(sess.sessionHash);persistAccountsSoon();return ack?.({ok:false});}socket.data.userId=u.id;socket.data.accountSessionHash=sess.sessionHash;persistAccountsSoon();ack?.({ok:true,expiresAt:sess.expiresAt,user:publicUser(u)});});
  socket.on("accountLogout",(_,ack)=>{if(socket.data.accountSessionHash)sessionStore.revokeHash(socket.data.accountSessionHash);socket.data.userId=null;socket.data.accountSessionHash=null;persistAccountsSoon();ack?.({ok:true});});
...
const cleanupTimer = setInterval(() => {
  ...
  if (sessionStore.cleanup(now)) persistAccountsSoon();
}, 60_000);
cleanupTimer.unref?.();
...
async function gracefulShutdown(signal){
  ...
  console.log(`${signal} received — flushing account persistence.`);
  try{await accountStore.flush(accountSnapshot);}catch(e){console.error("Final account persistence failed:",e.message);}
  try{await accountStore.close();}catch{}
...
async function bootstrap(){
  ...
  for (const room of rooms.values()) if (room.phase !== "lobby" && room.phase !== "roundEnd") scheduleBot(room);
  await configureScaling();
  server.listen(PORT,"0.0.0.0",()=>{console.log(`Kaali Ni Tidi v37 (${GAME_VERSION}) running on port ${PORT} · region ${REGION_ID} · accounts ${accountStore.mode()}`);});
}
