...
function deckCountFor(playerCount) {
  // 3–4 players use one deck. Every larger table uses two decks.
...
io.on("connection", socket => {
  ...
  socket.on("rankedJoin",(payload={},ack)=>{const me=userForSocket(socket);if(!me)return ack?.({ok:false,error:"Sign in to play ranked."});if(findRoomBySocket(socket.id))return ack?.({ok:false,error:"Leave your room first."});const pc=[3,4,5,6,7,8].includes(Number(payload.playerCount))?Number(payload.playerCount):4;removeFromRankedQueue(socket.id);const key=rankedQueueKey(pc),q=rankedQueues.get(key)||[];q.push({socketId:socket.id,userId:me.id,joinedAt:Date.now()});rankedQueues.set(key,q);ack?.({ok:true,position:q.length,needed:pc,season:currentSeasonId(),rating:ensureSeason(me).rating});io.to(socket.id).emit("rankedQueueStatus",{position:q.length,needed:pc});checkRankedQueue(pc);});
...
configureScaling().finally(()=>server.listen(PORT,"0.0.0.0",()=>{console.log(`Kaali Ni Tidi v19 running on port ${PORT} · region ${REGION_ID}`);}));
