  ...
  async (req,res)=>{
    try{
      ...
      try{
        ...
        console.error("Paytm status non-JSON response:", statusResp.status, statusRaw.slice(0,500));
        return res.redirect(`/payment-result.html?status=unavailable&id=${encodeURIComponent(orderId)}`);
      }
...
app.post("/api/paytm/initiate",async(req,res)=>{
  try{
    ...
    try{
      ...
      console.error("Paytm non-JSON response:", response.status, raw.slice(0,500));
      return res.status(503).json({
        error:"Paytm service temporarily unavailable. Please try again shortly."
      });
    }

    if(!response.ok){
      console.error("Paytm HTTP error:", response.status, data);
      return res.status(502).json({
        error:data?.body?.resultInfo?.resultMsg || "Paytm gateway returned an error."
      });
    }
