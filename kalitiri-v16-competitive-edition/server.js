...
app.get("/api/public-config",(_req,res)=>res.json({region:REGION_ID,season:currentSeasonId(),pushConfigured:Boolean(process.env.FIREBASE_PROJECT_ID),turnConfigured:Boolean(TURN_URL)}));
app.get("/api/admin/overview",(req,res)=>{if(!adminAllowed(req))return res.status(401).json({ok:false});res.json({ok:true,region:REGION_ID,analytics,rooms:rooms.size,users:users.size,rankedQueued:[...rankedQueues.values()].reduce((a,q)=>a+q.length,0),reports:reports.slice(-50),errors:clientErrors.slice(-50),tournaments:[...tournaments.values()].map(tournamentPublic)});});
...
function publicUser(u) {
  ...
  return { id:u.id, username:u.username, displayName:u.displayName, avatar:u.avatar, createdAt:u.createdAt,
    stats:u.stats || {}, rating:ss.rating || 1000, season, seasonStats:ss,
    achievements:u.achievements || [], friends:u.friends || [], settings:u.settings || {}, challenges:getDailyChallenges(u) };
}
...
function updateCloudRoundStats(room, i, won) {
  ...
  u.stats ||= {rounds:0,wins:0,losses:0,points:0,tricksWon:0,highestBid:0};
  u.stats.rounds++; won?u.stats.wins++:u.stats.losses++; u.stats.points += Number(p.lastAward||0);
  if(i===room.bid.bidderIndex) u.stats.highestBid=Math.max(u.stats.highestBid||0,Number(room.bid.current||0));
...
function storeReplay(room, bidderPoints, defensePoints, made) {
  const replay={id:crypto.randomBytes(8).toString("hex"),createdAt:Date.now(),code:room.code,round:room.round,playerCount:room.playerCount,ranked:Boolean(room.ranked),
    players:room.players.map(p=>({name:p.name,avatar:p.avatar,accountId:p.accountId||null,team:p.team,score:p.score,lastAward:p.lastAward})),bid:room.bid.current,trump:room.trump,bidderIndex:room.bid.bidderIndex,bidderPoints,defensePoints,made,
    events:(room.replayEvents||[]).slice(-300)};
...
function rankedQueueKey(playerCount){ return `${currentSeasonId()}:${playerCount}`; }
function checkRankedQueue(playerCount){ const key=rankedQueueKey(playerCount), q=rankedQueues.get(key)||[]; const live=q.filter(x=>io.sockets.sockets.has(x.socketId)); rankedQueues.set(key,live); if(live.length<playerCount)return;
  ...
  const firstUser=users.get(group[0].userId), code=roomCode(); const room=createRoomState(code,hostSocket.id,firstUser.displayName,firstUser.avatar,playerCount,firstUser.id);
  room.ranked=true; room.isPublic=false; room.matchConfirmation=true; room.players=[]; room.botDifficulty="hard"; room.replayEvents=[];
...
function createTournamentStage(t, entrants){ t.stageEntrants=entrants.slice(); t.stageWinners=[]; t.activeRooms=[]; for(let a=0;a<entrants.length;a+=4){ const ids=entrants.slice(a,a+4); if(ids.length<4){t.stageWinners.push(...ids);continue;} const connected=ids.map(id=>[...io.sockets.sockets.values()].find(s=>s.data?.userId===id)); if(connected.some(x=>!x)){ t.stageWinners.push(...ids.slice(0,2)); continue; }
    const u0=users.get(ids[0]), code=roomCode(), room=createRoomState(code,connected[0].id,u0.displayName,u0.avatar,4,u0.id); room.players=[]; room.tournamentId=t.id; room.tournamentRound=t.round; room.isPublic=false; room.botDifficulty="hard"; room.replayEvents=[];
...
function createRoomState(code, hostSocket, name, avatar, playerCount, accountId = null) {
...
function serializeRoom(room, viewerIndex, spectator = false, spectatorProfile = null) {
...
function startRound(room) {
  ...
  room.lastTrick = null;
  room.replayEvents = [];
  recordReplayEvent(room,"roundStart",{round:room.round});
...
function scheduleInGameReconnectExpiry(room, reconnectToken) {
  ...
  p.disconnectTimer = setTimeout(() => {
    ...
    addLog(room, `${player.name} did not reconnect in time; the bot keeps the seat.`);
    if (wasHost) assignNextHost(room);
    emitRoom(room);
    scheduleBot(room);
  }, RECONNECT_GRACE_MS);
...
io.on("connection", socket => {
  ...
  socket.on("accountLogin",(payload={},ack)=>{ if(!takeRate(socket,"accountLogin",10,60_000,"ip"))return rejectRate(ack); const u=findUserByUsername(payload.username);if(!u||!verifyPassword(String(payload.password||""),u))return ack?.({ok:false,error:"Incorrect username or password."});const token=issueSession(u.id);socket.data.userId=u.id;ack?.({ok:true,token,user:publicUser(u)}); });
  socket.on("accountResume",(payload={},ack)=>{const sess=accountSessions.get(String(payload.token||""));const u=sess&&users.get(sess.userId);if(!u)return ack?.({ok:false});socket.data.userId=u.id;ack?.({ok:true,user:publicUser(u)});});
  ...
  socket.on("rankedJoin",(payload={},ack)=>{const me=userForSocket(socket);if(!me)return ack?.({ok:false,error:"Sign in to play ranked."});if(findRoomBySocket(socket.id))return ack?.({ok:false,error:"Leave your room first."});const pc=[4,5,6,7,8].includes(Number(payload.playerCount))?Number(payload.playerCount):4;removeFromRankedQueue(socket.id);const key=rankedQueueKey(pc),q=rankedQueues.get(key)||[];q.push({socketId:socket.id,userId:me.id,joinedAt:Date.now()});rankedQueues.set(key,q);ack?.({ok:true,position:q.length,needed:pc,season:currentSeasonId(),rating:ensureSeason(me).rating});io.to(socket.id).emit("rankedQueueStatus",{position:q.length,needed:pc});checkRankedQueue(pc);});
  ...
  socket.on("quickMatch", (payload = {}, ack) => {
    ...
    const preferred = [4,5,6,7,8].includes(Number(payload?.playerCount)) ? Number(payload.playerCount) : 8;
    let room = [...rooms.values()].find(r => r.isPublic && r.phase === "lobby" && r.playerCount === preferred && r.players.length < r.playerCount);
    if (!room) room = [...rooms.values()].find(r => r.isPublic && r.phase === "lobby" && r.players.length < r.playerCount);
    ...
    if (!room) {
      ...
      const code = roomCode();
      room = createRoomState(code, socket.id, player.name, player.avatar, preferred);
  ...
  socket.on("createRoom", (payload = {}, ack) => {
    ...
    const code = roomCode();
    const room = createRoomState(code, socket.id, cleanName(payload?.name), cleanAvatar(payload?.avatar), pc, socket.data.userId || null);
  ...
  socket.on("reportPlayer", (payload={}, ack) => {
    ...
    const rec={room:found.room.code,reporter:found.room.players[found.index].name,target:found.room.players[targetIndex].name,reason:cleanChat(payload?.reason||"Player report"),ts:Date.now()};
  ...
  socket.on("rematch", (_, ack) => {
    ...
    if(humans.length && humans.every(x=>x.rematchReady)) setTimeout(()=>{ if(rooms.has(room.code)&&room.phase==="roundEnd") startRound(room); },450);
  });
...
configureScaling().finally(()=>server.listen(PORT,"0.0.0.0",()=>{console.log(`Kaali Ni Tidi v16 running on port ${PORT} · region ${REGION_ID}`);}));
